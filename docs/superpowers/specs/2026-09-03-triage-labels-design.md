...
- **Body hydration:** through the existing `MailService.getMessage` path (which caches `bodyText`/`bodyHtml` on the row) using the stored Zimbra token. A Zimbra failure for one message skips it this tick and is retried on subsequent ticks, but only up to `HYDRATION_FAILURE_LIMIT` (3) consecutive failures, tracked in an in-memory counter keyed by message id; on the 3rd consecutive failure the message is tombstoned (same as a failed extraction) so it stops being selected. The counter resets on a worker restart, which is an accepted tradeoff (a few extra retries after a restart, never persisted).
...
- **Purge:** runs hourly (not every tick — a full-table scan by `receivedAt` doesn't need per-minute freshness), gated by an in-memory `lastPurgeAt` timestamp compared against a 3.6M ms interval; when due, delete cards (and tombstones) whose message `receivedAt < now() - 90d`. Constants `CARD_BACKFILL_DAYS = 14`, `CARD_RETENTION_DAYS = 90`.
- **Observability:** one summary log line per non-empty tick (`classified N, failed M, purged P, skipped S`); each skipped message also logs a `warn` line with its message id and error message. The originally proposed "skipped-expired-token users" count was dropped — it isn't measurable from the tick's own query results without an extra query, so it was left out rather than approximated.

### 4. API surface

- `GET /mail/cards?ids=<id,id,…>` — batch card lookup for visible list rows (user-scoped, max 100 ids, returns `{ cards: { [messageId]: { label, importance, injectionSuspected } } }` — the UI needs labels, not full cards). `MailService.getCardsByIds` returns a label for any of the caller's own non-tombstone cards, regardless of the message's current folder — a deliberate deviation from the "Inbox/Sent within the window only" phrasing below: labels remain accurate wherever a message lives, and cards still age out via the 90-day purge, so scoping the batch lookup to folder/window would only hide correct labels without a compensating benefit.
