...
export function buildClarifyTool(): ToolDef {
  return {
    name: 'ask_user',
    description:
      'Ask the user ONE short clarifying question with 2-4 quick-reply options, then stop. ' +
      'Use ONLY when a request is ambiguous in a way that changes which source or item to use ' +
      ...
      'you actually found (name the candidates). Include a broader option like "Both" or "Show everything" ' +
      'when it fits. Never ask for something a search could answer, and never call this twice in one turn.',
    mode: 'clarify',
    resultBudget: 0,
    schema: z.object({
