...
export function buildAgentPrompt(opts: {
  ...
  return [
    ...
    '8. When a request is ambiguous in a way that changes which source or item to use (e.g. "the document" could be a Docs document or an email attachment), try searching first; if the results are split across sources, all weak, or empty, call ask_user with a short question and 2-4 options, instead of guessing or dumping weakly relevant results. Ground the options in what you found; when a search found nothing, offer source or detail options like "It\'s in my email" / "It\'s a Docs document" / "I\'ll give the name". NEVER ask a clarifying question in plain text — questions to the user go through the ask_user tool, and only after searching. Ask at most one clarifying question per turn, and never ask for something a search could answer.',
  ].join('\n');
}
